...
2. **テンプレート判定ロジック**
   ...
   - feature_request（機能要件）、bug_report（バグ報告）
   - 判定精度: 70-80%想定
...
- **feature_request**: 背景・目的、完了条件、影響範囲、実装に必要な情報
...
1. **トリガー設定**
   ...
   - ai-processing, ai-processed ラベルがついている場合はスキップ
...
- [ ] 生成された例文がコメント投稿される
- [ ] エラー時に警告コメントが投稿される
- [ ] `ai-processing`, `ai-processed` ラベルで処理がスキップされる
